...
function getRolUser(
  ...
  const user = dataUserAll.find(
    (item) => item.id === selectedStep.idUsuario
  )?.nombre;

  // get rol from datausersall by props.selectedStep.idRol
  const rol = dataRolesAll.find(
    (item) => item.id === selectedStep.idRol
  )?.nombre;

  console.log(selectedStep);

  console.log("rssol", dataRolesAll);
  console.log("user", dataUserAll);
...
const FormNewPaso = withFormik<MyFormProps, FormValues>({
  // Transform outer props into form values
  mapPropsToValues: (props) => {
    return {
      name: props.selectedStep?.nombre || "",
      description: "",
      content: props.selectedStep?.descripcion || "",
    };
  },
